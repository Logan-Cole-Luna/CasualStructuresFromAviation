...
def main():
    ...
    # Per-class metrics — only available if eval_report has classification_report
    ...
    if class_report and label_map and not args.skip_distilbert:
        ...
        print('  Per-class metrics not in saved report — run eval.py to generate confusion/per-class plots.')
    ...
    if rule_triples or dep_triples or full_llm_triples:
        llm_sample_n_for_radar = len(llm_cache) if llm_cache else sample_n
        plot_radar_extraction(
            rule_triples, dep_triples, full_llm_triples,
            stats_rules, stats_deps, stats_llm,
            sample_n=max(sample_n, llm_sample_n_for_radar),
            plots_dir=plots_dir,
        )
    ...
    llm_for_comparison = full_llm_triples
    plot_cross_model_comparison(
        rule_triples, dep_triples, llm_for_comparison,
